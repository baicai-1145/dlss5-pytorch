...
def ln_fwd(self, x):
    ...
    if m == "W4" and (Hp % self.ws_dummy == 0 and Wp % self.ws_dummy == 0):
        ...
        return y.view(B,Hp,Wp,C)
    if m == "T2":
        s2 = (x*x).mean(-1, keepdim=True).clamp_min(FLOOR)
        return x * torch.rsqrt(s2) * self.weight[wsh]
    if m == "T3":
        s2 = (x*x).mean(-1, keepdim=True).clamp_min(FLOOR)
        y = x * torch.rsqrt(s2) * self.weight[wsh]
        return y + self.bias[wsh] if self.bias is not None else y
...
def comb_f23():
    ...
    return out
for name, mode in [("T2 tokRMS mean   ","T2"), ("T3 tokRMS mean+b ","T3")]:
    MODE["m"] = mode
    c16 = forward_clean16()
    f2, f3 = comb_f23()
