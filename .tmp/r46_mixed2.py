"""R46: mixed-domain matrix WITH execution counters. Decisive check:
does W1 actually execute at enc234/bn? If yes but output == baseline,
investigate why. Groups: per-stage (enc.0/1/2/3/4, bn, dec.0-4, expands)."""
import os
...
def build():
    ...
    return m
m = build()
print("tagged groups:", sorted({getattr(mod,'group','?') for mod in m.modules() if isinstance(mod, torch.nn.LayerNorm)}))
def clean16():
...
def comb_f23():
    ...
    return out
SEL["groups"] = ("enc.2","enc.3","enc.4")
c16 = clean16(); f2, f3 = comb_f23()
print(f"W1@enc234       : clean16={c16:+.4f} f2={f2:+.4f} f3={f3:+.4f} exec={dict(COUNT)}", flush=True)
COUNT.clear()
SEL["groups"] = ("dec.2",)
c16 = clean16(); f2, f3 = comb_f23()
print(f"W1@dec.2 only   : clean16={c16:+.4f} f2={f2:+.4f} f3={f3:+.4f} exec={dict(COUNT)}", flush=True)
COUNT.clear()
SEL["groups"] = ("dec.0","dec.1","dec.2","dec.3","dec.4")
c16 = clean16(); f2, f3 = comb_f23()
print(f"W1@dec-all      : clean16={c16:+.4f} f2={f2:+.4f} f3={f3:+.4f} exec={dict(COUNT)}", flush=True)
