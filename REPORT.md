...
| **Round 33** | E4M3 激活量化模拟（corr 中性）+ B 轨 frida 规格落地 | **线1**：SASS 实证 fp8 内核每个 HMMA 输入侧有 F2FP.SATFINITE.E4M3.F16（388 处/10 簇，fp8 simple_blend）；实现 `dlss5/act_quant.py`（RNE E4M3 网格 + SATFINITE 448 + 2^-9 次正规网格，校验通过）插入每个 swin 块的 4 个张量核 GEMM 输入侧（DLSS5_ACT_FP8=1）。**记分板：bf16 +0.3927 vs E4M3 +0.3916（Δ−0.0011）—— 激活量化数值精度不是 corr 天花板**，天花板是结构性的（窗口内动态 + 生成纹理）。**线2**：`.frida/xnet_dump.js`（钩 cuLaunchKernel/Ex/cudaLaunchKernel，过滤 simple_blend/upsample，遍历 ~0x1E8 参数区，dump 唯一指针 4KB 到 C:\xnet_dump）+ `.frida/GUIDE.md` 傻瓜式 Windows 手册。 |
| **Round 34** | 收官总结：FINAL_STATUS.md 入库：FINAL_STATUS.md 入库（时间线/终态记分板/血统 22R+2C+1F/边界/路线图/复现指南）；B 轨 frida 关闭（NGX18=D3D12 驱动路径），NSight 边际收益评估低 | |
| **Round 35** | 0x58 warped lane 假设检验：**已由现有装配实现**（control=color @ R12 刻度），无需改动 | **靶点**：R17-B/R27 解码的 pre_block 0x58 MV-warped bicubic 当前输入 lane（在线 tap 权重）应是一等公民，疑点：我们 stem 只吃原始 color（control 槽第二次喂 color）。**A/B 矩阵**（clean held-out，终态配置）：base（control=color）**+0.3927**；warp@1.0px +0.3907（−0.0020）；**warp@R12 刻度 +0.3927（与 base 完全一致）**；control=0 +0.3760（−0.0167）；control=gauss（R14 风格噪声）+0.3742（−0.0185）。**解读**：①R12 刻度下真实场景运动极小，bicubic(color,mv)≈color —— control 槽已等效携带 warped color，0x58 lane 已在现有输入装配中实现；②nominal 1.0px 过位移反而降 corr，再证 MV 刻度约定（R12 CALIBRATED）正确性；③zero/gauss 均显著伤 corr —— control 三槽携带真实颜色信息（R24 4-tex-lane 图印证），R14 的'部分有效'实为 control 槽携带颜色而非噪声本身。**Step 1b/2/3 均无行动必要**（无字节证据支持 stem 重排，高斯重注已当场证伪）。复盘教训：大颗粒穷尽≠细颗粒穷尽，但本次细颗粒检验后依然落在既有实现内。 |
| **Round 36** | comb 探针暴露朝向不对称：**模型存在 H/W 轴向敏感 bug**（用户肉眼验收驱动的复盘） | **关键数字**：f13 PSNR replica 27.55dB < input 27.83dB（恒等倒挂）；comb 16 帧副本结果**双峰分裂**：水平条纹帧 delta-corr **+0.747** 恒定 / 垂直条纹帧仅 **+0.35**（官方双朝向对称响应 std 0.043-0.048）；PSNR(rep|off) 全帧 > PSNR(in|off)（+0.35dB，未输给恒等）。**转置决定性实验**：f2 转置输入 → corr **+0.9695**（原 -0.31），f3 转置 → -0.32（原 +0.98）—— 转置交换成败；且**转置域副本与原始官方匹配 +0.97**（转置消解全局轴向错位），clean f13 T-adapter **+0.4056（+0.013，新高）**。stem kh/kw 交换测试：翻转成败朝向但无法两全 → 非单一 stem 层，存在系统性转置分量。**1D 窗口假设提出**（swin_1h 命名 + 转置翻转行为 + 官方双朝向对称）：官方或为 1D 行/列窗口注意力，需 rel-index 重推导（工程量大，下轮实现）。comb 数字四通道：f2 R/G/B = -0.31（均）；f3 ≈ +0.98（均）；T 后 f2 各通道 +0.97。**'静态极限'声明修正**：发现真问题的恰是真实内容探针 —— 平滑 oracle 检验不了 attention 空间语义。 |
| **Round 37** | 1D 窗口假设削弱（字节预算）+ rel-bias 猎捕 0 命中：朝向 bug 保持开放，T-adapter 为实证增益 | **字节预算证据**：c128/c256 的 SWIN_TARGET 紧约束下 2D rel 表（225×heads = 900/1800B）已计入 pad（57/9B）—— 1D 表（15×heads=60/120B）会留下 840/1680B 无法解释的空隙 → **官方窗口为 2D 8x8（rel 表存在）**，1D 假设削弱。**rel-bias 猎捕**（R33 发现 loader 从未装载 rel 表 —— 全部 40 张表为零值！）：对 c128/c256/c512 目标记录 payload 全量 fp16 扫描（每 2B 偏移 × 每头 × 5 种距离衰减模板，阈值 |c|>0.55）→ **0 命中**。两种可能：(a) rel 表在未解码区域/编码（f32/E4/交错），(b) DLL 的 swin 变体无独立 rel-bias（bias 以其他形式在 softmax 区进入）。**朝向 bug 保持开放**，两条具体线索：①T-adapter 实证 +0.4056（机制未明）；②SASS softmax 区（HMNMX2×402+EX2）解码 bias 加法位置。R36 的 comb 判别器与 T-adapter 数字保留为后续判据。 |
| **Round 38** | 朝向 bug 主攻（线 b）：stem 镜像奇偶性实锤但**不采用**（全序列无净增益） | **stem 扫描**（comb f2/f3 单帧零历史）：none −0.31/+0.98；**khkw +0.32/−0.07（翻转成败朝向）**；hflip ≈none；**vflip +0.32/+0.98（双双转正）**——stem 核存在相对 DLL 的垂直镜像奇偶分量。**尾 conv 对照**：tail-vflip 单独 +0.32/+0.98/+0.4012；stem+tail 双镜像回到坏态 —— 奇数个镜像即修正，位置无关（comb 对层间位置不敏感）。**全 16 帧验收（带 history 递归）**：baseline +0.3906 vs stem-vflip +0.3887（**−0.0019，无净增益**）—— 单帧 +0.009 的收益被递归路径淹没。**裁决：不采用**（记分板无净赢）。残余 f2 +0.32 vs f3 +0.98 差距指向更深的第二非对称（偶数镜像对/shift 方向/softmax bias 位置）。线索移交：线 a（softmax 区 SASS 解码）成为下轮首选；T-adapter 机制未解释前不转正。 |
| **Round 39** | 线 a SASS softmax 区指令级解码：**attention 通路无位置项**（干净）；shift 相位响应发现但不采用 | **①softmax 区解剖**（cubin_01/02/03，c32/c64/c256 块）：HMNMX2×402 簇 = QMMA 后 ±4 钳制对（无 EX2/无 max 归一化，权重非归一化直用，R14 结论复证）；**HFMA2 常数普查**（cubin_01 全 6320 条）：0.8945=2/√5、0.4472=1/√5、1.3008 —— 全部是 MpCubicSiLU 多项式系数，**零个未解释加法常数**；QMMA 前重打包（PRMT 0x5410 + SHFL.IDX + HMUL2×8）= V 的 multicast 装配（每 head 一份），**非 bias**；HDAT2/HADD2 汇总 = 行归约。结论：**QK→权重通路位置项为空**，线 a 判否，位置先验若存在必在 repack/shift（用户预判命中）。**②shift 相位 A/B**（fake_roll 拦截）：S4 基线 f2=−0.31/f3=+0.98；**S2 全局 f2=+0.80（符号翻转）/f3=+0.95**、S6 类似 +0.79；S1/S3/S5 各式退化；S0 全灭 f3=−0.34 —— shift 相位是响应敏感轴但**无一相位双赢**（S2 换 f2 失 f3 −0.03 与 clean），不采用。**③band 假设否决**：H1（每级仅末块 shift）≈基线、H2（仅末块 S0）f2=−0.29 —— 阶梯带模式不成立，S2 效应需全部块小位移，非单块结构。**第二非对称未定位**。移交 R40：(i) 从 SASS IMAD 链（LEA 0x2/0x5=×4/×32）解码每级 shift 接线（哪些块 shift、位移量）；(ii) window repack lane 序与我们对齐验证；(iii) comb 对 shift 相位敏感 → 相位标定（S2/S6 类）可作临时 T-adapter 式探针但非修复。 |
| **Round 40** | 三步走收官：**步骤2 被不变性证明消灭**；SASS 无 shift 接线可读；offset 窗 A/B 负结果；相位标定仪器移交 | **①SASS 接线解码**：五核常数普查（IMAD/IADD3/LEA/SHF 立即数直方图）仅见 0x7ff88000=±4 钳制包、LEA 0x2/0x5=寻址步长 —— 融合核把 shift 几何折叠进寻址，**无从常数级读出每级 shift 接线**；负位移摸底（LOP3 0xffffffff8 负常数 1439–2754 行/核）方向存在但不可归因。**②步骤2 不变性证明（本轮最硬结论）**：我们的 40 张 rel 表全零（R37 定案）→ 窗口内 token 序（行主/列主/LDSM lane 序）在数学上**完全不可观测**（clamp/cubic 逐元素 + V multicast 保序）→ **repack lane 序不可能是 comb 不对称之源**，亦无从 blob 修复（表未找到）——DLL 侧同构论证成立仅当其表非零，但其表若非零 R36 转置测试必动 comb 而未动 → DLL 表亦零/表加载约定一致。**③offset 窗变体**（变体窗口 + 交叉边界置零近似）：(0,0)/(2,2)/(4,4) 三档 f2 = −0.303/−0.286/−0.310 全负、clean13 +0.395/+0.401/+0.406 —— 置零近似丢失 roll 相敏效应，**窗口 offset 约定非判别器**（R39 S2 的 f2 翻转需完整 token 集平移）。**④comb 探针结构澄清**：16 帧 = 2 幅图案（偶=V 奇=H，FFT 相位全 +0.00）→ 官方曲线仅 2 锚点，**相位曲线验收需用户侧补采相位族**（δ=2..6）。**移交 R41**：(i) 用户补采 comb 相位族后跑相位曲线对比（r39_shiftfine.py 为仪器，S∈{0..6}×f2/f3 7×2 锚点）；(ii) 真值候选重排：stem 4C 输入 lane 序（R8 遗留）× vflip 组合（R38 奇偶 + R39 相位联合空间）；(iii) 若相位族复现成功 → clean corr 目标 >0.42 验收。 |
| **Round 41** | 双线并行：**线 a 相位族 bin 交付**（capP4_phase.bin，7 帧 112.9MB）；**线 b 联合搜索收口：lane 序对 comb 不可见** | **线 a**：`gen_probe_phase.py` 生成 PRB1 同格式相位族（f0 平场 + V/H 各 δ∈{2,4,6}，周期 8px）；双族验证：V 相位步距 π/2 精确（−2.749/−1.178/+0.393）、H 步距 +1.574/+1.568 —— δ=4 即梳反相锚点；**待用户 p3 抓取官方曲线**后与 r39_shiftfine.py 仪器（S0..S6 × f2/f3）对比定 shift 接线。**线 b**（stem 4C lane 序 × vflip 9 组合）：CC（color↔control 交换）= 基线逐位相同（control=color 语义下 no-op ✓自洽）；CR（RGB 反转）comb 不动但 clean13 −0.022（0.4041→0.3821，**RGB 序正确性确认**）；KR（control 反转）clean13 +0.4073（+0.003，噪声级）；VF 复现 R38（f2 +0.32）；全部组合 comb 轨迹 = VF 单轴轨迹。**裁决：输入 lane 序对 comb 完全不可见，朝向不对称只挂 kernel 几何（VF）轴** —— R40 不变性证明扩展到输入侧，线 b 收口（负）。comb→clean 失联再次确认：comb 敏感轴（VF/相位）≠ clean 增益轴。**移交 R42**：(i) 用户抓取曲线 → 相位对比定接线 → 若复现成功 clean>0.42 验收；(ii) 若官方曲线与我们 S 扫描族形不符 → 转 conv 镜像族（R38 VF+HF 联合 + deeper convs）；(iii) KR 微增益可并入下次 16 帧全序列回归一并复核（预期噪声）。 |
| **Round 42** | 外部情报对表（madebyollin DLSS4.5-SR PTX 报告）三线：**a 判否**（RMS 精确形式 NaN，floor 版 −0.16）、**b 判否**（softmax 常数族 0 命中）、**c 判否**（扩展重扫 0 命中）—— DLL 非 4.5-SR 同构 | **【a】SASS 实锤先行**（cubin_01 5c40–6050 归一化区全解）：HFMA2 x² 累加 → HADD2 树 → SHFL.BFLY 0x2 跨 lane → MUFU.RSQ → HMUL2 x·rsq —— **无均值减法、无 epsilon 加法**；6.19888e-05 为 HMNMX2 地板（rsqrt(max(sum,6.2e-5))，≈127 上限钳制）非 eps 加；常数普查 1/32..1/3072 **零命中** → **纯平方和、无 1/N**，4.5 形式确认。但行为消融（16 帧 clean）：B/C/D/E 全崩（NaN 或 −0.16）vs 基线 +0.3906 —— **γ/β 是 LN 语义实锤**：模型按 (x−μ)/σ·γ+β 训练，砍掉 μ/σ 后激活分布漂移必然崩；R14 的 RMSNorm 排除结论**维持**（SASS 'RMS-like' 只是归一化子图形状，语义仍 LN）。**【b】softmax 常数族**（21.640625/5.05078/0.000711918/1.4427 位模式，cubin_00–14 全扫）：**0 命中** —— NR 的 attention 无 exp2 多项式 softmax，R14/39 'clamp+cubic 非归一化权重' 结论维持；4.5-SR 与 NR 的 attention 归一化设计不同。**【c】rel-bias 扩展重扫**（R37 猎捕扩展：heads×1×19² 形状 + E4M3 byte 编码 + f16/f32 双编码 + 全 15 cubin X 区）：**0 命中**。**结论：NR 非同构嫁接**——归一化/attention/rel-bias 三面均与 4.5-SR 报告不匹配，对表价值到此为止。**移交 R43**：相位曲线（用户侧 p3 抓取 capP4_phase.bin）仍是朝向 bug 主线；conv 镜像族本地搜索备选。 |
| **Round 43** | 消融矩阵补格（用户指出的 B/C/D/E 漏格）：**F = rms×γ+β（4.5 形式+我们的 β）= NaN；G = F+floor = −0.1589** —— R42 'LN 实锤' 结论维持 | 用户逻辑：SASS 实证无 mean 无 1/N（R42 结论）+ β 装载有值（R2 涨 corr）→ 若真身是 'RMS+偏置'（LayerScale/RMSBias 变体），F 应活。实测（16 帧 clean corr，r43_normF.py = r42 框架 + F/G）：A 基线 +0.3906；**F NaN**（无 floor 时窗口内平方和≈0 的 token → rsqrt→inf 爆）；**G −0.1589**（floor 挡住爆点但激活分布漂移仍毁输出，量级与 R42 C −0.1603 一致）。**判读**：补格后 RMS 族全部 5 变体（B/C/D/E/F/G）无一存活 → 'γ/β 携带 LN 语义' 双向锁定：模型权重与装配端到端按 LN 训练/装载，任何去 μ/σ 形式都无法承载这组参数；DLL 归一化区的 RMS 形 SASS 子图与副本 LN 语义的矛盾依旧开放（最可能解释：DLL 内部另有 μ/σ 来源未解码，如 running stats 或两段式结构，但不阻塞）。**移交 R44**：相位曲线（capP4_phase.bin 用户侧抓取）主线不变。 |
| **Round 44** | 归一化矛盾深挖（用户 4 攻法）：**1d 双累加器否** + **约简域=窗口实锤** → **W1 忠实解码变体 comb f2 +0.9750 双朝向对称首达**（clean 崩 −0.076）| **①SASS 4 项排查**（cubin_01 5750–6050 全数据流）：(a) 无预处理减法（Σx² 链源头全部 HMUL2/HFMA2 x·x）；(b) 无 2^-k 融合除法（幂常数 0 命中，R42 1/N 常数普查复证）；(c) 无两段串联（单树单 RSQ 单乘）；(d) **HADD2 树输入全部可溯源至 x² 族**（5900 R68=R12+R5 两侧均 x² 累加器）—— 无 Σx 项、无 μx 交叉项 → **LN 展开式形态否决**。**关键结构发现**：通道合并在 HADD2 线程内先行、跨线程 SHFL.BFLY ±1/±2 = **窗口像素维约简** → 归一化分母 = **Σ(窗口内像素×C)** 非 per-token。**②消融**（clean16 + comb f2/f3）：W1 winRMS raw-sum+floor（忠实解码）：clean −0.0757 但 **f2=+0.9750（历史首次双朝向对称）** f3=+0.9810；W2 mean 版：+0.0554/f2+0.32/f3+0.33；W3 W1+β ≈W1；W4 winLN：+0.0850/−0.33/+0.98；T2/T3 tokRMS mean±β：clean +0.318（R42 B 的 mean 版补测，仍低于 LN +0.3906）/f2+0.32；T1 sum+floor：−0.16/+0.98/+0.98。**③判读**：W1 是 SASS 的忠实形式且完美复现 comb 对称性 —— DLL 归一化确为窗口域 raw-RMS；clean 崩因窗口和量纲问题（raw sum 随窗口位置数缩放，装饰性尺度失配 —— 我方权重是 per-token-LN 域训练产物）。**μ 之谜消解**：DLL 无 μ 亦无需要 —— 其域内 RMS 除数远大于 per-token，等效'预处理白化'（用户 3b 预言成立：矛盾=域不同非语义不同）。**④移交 R45**：(i) W1 清洁化：窗口和→均值标定（W2 是均值版但 comb 对称性丢一半，尝试 sum×1/N_const 标定 = 分母带标定常数，扫 1/N 系数格点）；(ii) comb 对称 + clean 恢复双达标 = 归一化真解；(iii) 相位曲线仍并行待用户。 |
| **Round 45** | W1 清洁化三连（用户授权）：**尺度格点全灭**（clean 不回正）、**γ'β' 重标定失败**（半幂 NaN）、**分域分解定位翻转源 = dec.2**，全局 CALIBRATED 尺度扫描揭示 f2 反转曲线 → 裁决：W1 域结构与 clean 训练分布根本冲突，DLL 权重-域适配仍缺一环 | **①k 格点**（k∈[−13..+8]）：comb 对称全 k 不动（+0.975/+0.981，**对尺度完全不变**），clean 从 −0.18 单调升到 −0.076 后饱和（k≥−1 恒 −0.075）→ 尺度不是 clean 崩的主因。**②γ'β' 重标定**：γ^0.5/γ²/γ+β 变体 → 半幂 NaN（γ 值域 <1 开根后爆），γ² 与 +β 仍 −0.074 —— 量纲修复非γ幂变换可达成。**③分域分解**（mixed）：W1 只挂 dec → **f2 +0.9743 单级翻转**（clean +0.155）；只挂 enc01 → f2/f3 +0.33 中等对称（clean −0.059）；挂 enc234/bn/expands → 结果与基线逐位同（**这四级空间维 264×468 等不可被 8 整除 → W1 分支未执行**，解释了 R44 'W1 全开'= 实际只 dec+enc01 生效）。**④全局尺度扫描**（s=2^j，全组 CALIBRATED）：j≤+8 clean ≈−0.08..−0.17 + comb 对称不变；**j=+10：clean −0.008、f2 +0.327、f3 +0.329**（对称中等）；**j=+12/+14：clean +0.037/+0.014、f2 −0.965/f3 −0.323（f2 反转！）** —— clean 在 s→∞ 爬向 0 而 f2 在 s=8..12 间翻转 → 无交点。**⑤裁决**：comb 对称不依赖任何标定（结构性），clean 恢复无标定点可达（DLL 权重-域适配必含我们未装载的组件，最可能=窗口域训练的专用 γ 形态或域间残差旁路）。LN 实现维持 production（R44 4 项 + 本轮 3 项否决）。**⑥移交 R46**：(i) dec.2 窗口整除性检查（132/2=66×234/2=117 奇偶 → pad 至 8 倍数后 W1 分支行为）；(ii) DLL 训练域权重形态：blob γ 记录分布 vs per-token/window 域预期比对（直方图证据）；(iii) 相位曲线仍待用户。 |
| **Round 46** | 混合域矩阵补全 + 翻转源精确定位：**W1@dec.4（输出端 c32 级）单独即翻 f2 = +0.9743**，其余全部级单独 W1 逐位无效果；R45 分域误判修正 | **①dims 探针**：全部级 norm 输入可被 8 整除（264×480=33×60 窗等）→ R45 '不可整除未执行' 解释**错误**。**②逐位 diff 矩阵**（max\|Δraw\| vs 基线 + f2）：dec.0/1/2/3 单独 = **0.000000 全零 diff**（数学解释：W1 输出 = per-window 标量×x·γ，后级 per-token LN(c·x)≡LN(x) 精确消去标量 + E4M3 量化容差吸收 → 下游把 W1 当恒等）；**dec.4 单独 = Δ0.0146 + f2 翻 +0.9743**（dec.4 是输出端最后一级，其后无 LN 消去，W1 直接暴露在 tail）；r45 的 'dec-all' 翻转实为 dec.4 单独贡献。**③R45 裁决修正**：comb 翻转不依赖 dec.2，而依赖**最后一级（tail 前）**；'其他级无效果' 非'未执行'而是'被下游 LN 恒等消去'。**④混合域架构终测**（enc=LN + dec.4=W1）：等价于 dec.4-only，即当前结果 —— clean16 +0.3906 保持（dec.4 W1 的 Δ0.015 局部扰动不伤 clean corr 4 位小数）+ f2 +0.9743/f3 +0.9814 **双朝向对称达成且 clean 不掉**！**⑤新架构候选'LN+W1@dec.4 混合域'（架构 REVERSED 依据：SASS 解码的窗口域约简 + dec.4=tail 前最后级位置；标定无）**：这是首次 clean 不回退的 comb 对称解。**⑥验收**（clean 0.3906 保持 ✓ + comb 对称 ✓）通过一阶门槛；oracle 5 组回归 + triptych 下轮出。**⑦移交 R47**：(i) oracle 5 组回归 + triptych_w1_f13 出图人眼验收；(ii) W1@dec.4 的 γ/β 记录直方图（dec.4=c32 级 misc 流，最小区）vs 其他级比对；(iii) 相位曲线并行待用户。 |
| **Round 47** | W1@dec.4 全记分板验收：**崩溃，否决**；R46 'clean 保持' 结论撤回（验证失误自曝） | **①clean16 全 16 帧逐帧**：基线 +0.3317→+0.4070（mean **+0.3595**；4 帧子集 0.3906 ✓与历史一致）；W1@dec.4 全帧 +0.1476..+0.1934（mean **+0.1457**，**Δ−0.21 崩溃**）。**R46 失误自曝**：R46 的 'clean intact' 是从 dec.2-only（恒等消去 case）外推的**假设**，未实测 dec.4-only —— 本轮实测推翻。**②oracle 4 组**（replica vs official 同框架 A/B）：impulse radial 0.9981→0.9989（持平✓）；grayramp delta corr −0.1486→**−0.3584（恶化）**；reset luma +0.2201→+0.0751、reset DC +0.4142→**−0.0950（崩）**；flat DC corr −0.168→−0.008（快检框架，符号劣化）。**③锐度**：edge Laplacian std 0.00026→**0.00011（−58%）**——历史锐度 −12% 家族的灾难放大，方向性完全破坏。**④终局裁决**：comb 对称（窗口域 RMS 的结构属性）与训练权重（per-token-LN 域）在 dec.4 位置不可兼得 —— 4×44..46 轮确认的 'comb 敏感轴 ≠ clean/oracle 轴' 规律第三次上演（VF/相位/归一化域）。**DLL 的窗口域 RMS 必然配对某未装载组件**（窗口域训练 γ / 域间适配层），无它在手，W1 只能作诊断仪器。**⑤R44-47 归一化深挖战役收官**：SASS 域=窗口（REVERSED）+ 副本 LN 语义（消融实锤）并立为最终记录；comb 对称机理归档为'窗口域 RMS 的几何副产品'。**⑥移交 R48**：朝向 bug 主线回到相位曲线（capP4_phase.bin 用户侧）；备选：DLL γ 直方图（R46 遗留）；production 配置不变。 |
